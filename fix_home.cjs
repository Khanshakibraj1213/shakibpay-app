...
const section3Index = code.indexOf('{/* Section 3: Auto Banner Offer Slider */}');
const walletTabIndex = code.indexOf('{/* === WALLET TAB VIEW === */}');

if (section3Index !== -1 && walletTabIndex !== -1) {
  const newHomeCode = `                  {/* --- NEW GRID & SECTIONS MATCHING SCREENSHOT --- */}
                  ...
                  <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-indigo-700 via-purple-700 to-indigo-800 text-white shadow-lg p-5 flex items-center justify-between cursor-pointer active:scale-[0.98] transition-transform" onClick={() => { setOffersMode('internet'); setActivePanel('offers'); playAudio('click'); }}>
                    <div className="absolute right-0 top-0 w-32 h-full opacity-20 pointer-events-none">
                       {React.createElement(require('lucide-react').Wifi, { className: "w-full h-full text-white", strokeWidth: 0.5 })}
                    </div>
                    <div className="space-y-2 relative z-10 text-left">
                      ...
                      <button className="mt-1 px-4 py-1.5 bg-indigo-900/50 hover:bg-indigo-900/70 border border-indigo-400/30 rounded-full text-[10px] font-black uppercase tracking-wider flex items-center space-x-1.5 transition-colors">
                        ...
                        {React.createElement(require('lucide-react').ArrowRight, { className: "w-3.5 h-3.5" })}
                      </button>
                    </div>
                    <div className="relative z-10 w-20 h-20 bg-indigo-500/30 rounded-full flex items-center justify-center backdrop-blur-sm border border-indigo-400/20">
                      {React.createElement(require('lucide-react').Wifi, { className: "w-10 h-10 text-white" })}
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className={\`p-3 rounded-2xl border flex flex-col justify-between space-y-3 \${theme === 'dark' ? 'bg-[#141A28] border-[#1D253B] shadow-lg shadow-[#141A28]/50' : 'bg-white border-neutral-150 shadow-sm'}\`}>
                        <div className={\`w-8 h-8 rounded-full flex items-center justify-center \${theme === 'dark' ? 'bg-blue-500/20' : 'bg-blue-50'}\`}>
                          {React.createElement(require('lucide-react').Wallet, { className: \`w-4 h-4 \${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'}\` })}
                        </div>
                        <div>
                      ...
                      <div className={\`p-3 rounded-2xl border flex flex-col justify-between space-y-3 \${theme === 'dark' ? 'bg-[#141A28] border-[#1D253B] shadow-lg shadow-[#141A28]/50' : 'bg-white border-neutral-150 shadow-sm'}\`}>
                        <div className={\`w-8 h-8 rounded-full flex items-center justify-center \${theme === 'dark' ? 'bg-emerald-500/20' : 'bg-emerald-50'}\`}>
                          {React.createElement(require('lucide-react').ArrowRightLeft, { className: \`w-4 h-4 \${theme === 'dark' ? 'text-emerald-400' : 'text-emerald-600'}\` })}
                        </div>
                        <div>
                      ...
                      <div className={\`p-3 rounded-2xl border flex flex-col justify-between space-y-3 \${theme === 'dark' ? 'bg-[#141A28] border-[#1D253B] shadow-lg shadow-[#141A28]/50' : 'bg-white border-neutral-150 shadow-sm'}\`}>
                        <div className={\`w-8 h-8 rounded-full flex items-center justify-center \${theme === 'dark' ? 'bg-amber-500/20' : 'bg-amber-50'}\`}>
                          {React.createElement(require('lucide-react').Users, { className: \`w-4 h-4 \${theme === 'dark' ? 'text-amber-400' : 'text-amber-600'}\` })}
                        </div>
                        <div>
                      ...
                      <div className={\`p-3 rounded-2xl border flex flex-col justify-between space-y-3 \${theme === 'dark' ? 'bg-[#141A28] border-[#1D253B] shadow-lg shadow-[#141A28]/50' : 'bg-white border-neutral-150 shadow-sm'}\`}>
                        <div className={\`w-8 h-8 rounded-full flex items-center justify-center \${theme === 'dark' ? 'bg-purple-500/20' : 'bg-purple-50'}\`}>
                          {React.createElement(require('lucide-react').CheckCircle, { className: \`w-4 h-4 \${theme === 'dark' ? 'text-purple-400' : 'text-purple-600'}\` })}
                        </div>
                        <div>
...
`;
  ...
  // We need to replace require('lucide-react') with direct variables since this is inside App.tsx which has imports.
  let cleanedNewHomeCode = newHomeCode.replace(/require\('lucide-react'\)\./g, '');
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/require\('lucide-react'\)\[([^\]]+)\]/g, 'eval($1)');
  
  // Actually, we can just replace the whole eval/require with simple variable mapping. Let's just string replace them.
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/eval\(serv\.icon\)/g, 'serv.icon');
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/eval\(item\.icon\)/g, 'item.icon');
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/eval\(act\.icon\)/g, 'act.icon');
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/"Plus"/g, "Plus");
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/"Send"/g, "Send");
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/"Package"/g, "Package");
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/"Smartphone"/g, "Smartphone");
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/"Receipt"/g, "Receipt");
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/"Building2"/g, "Building2");
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/"Users"/g, "Users");
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/"ShoppingBag"/g, "ShoppingBag");
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/"Coins"/g, "Coins");
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/"Flame"/g, "Flame");
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/"Rocket"/g, "Rocket");
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/"Activity"/g, "Activity");
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/"Shield"/g, "Shield");
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/"Wallet"/g, "Wallet");
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/"CreditCard"/g, "CreditCard");
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/"Fingerprint"/g, "Fingerprint");
  
  // React.createElement replaces
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/\{React\.createElement\(Wifi,\s*\{([^\}]+)\}\)\}/g, '<Wifi $1/>');
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/\{React\.createElement\(ArrowRight,\s*\{([^\}]+)\}\)\}/g, '<ArrowRight $1/>');
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/\{React\.createElement\(Wallet,\s*\{([^\}]+)\}\)\}/g, '<Wallet $1/>');
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/\{React\.createElement\(ArrowRightLeft,\s*\{([^\}]+)\}\)\}/g, '<ArrowRightLeft $1/>');
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/\{React\.createElement\(Users,\s*\{([^\}]+)\}\)\}/g, '<Users $1/>');
  cleanedNewHomeCode = cleanedNewHomeCode.replace(/\{React\.createElement\(CheckCircle,\s*\{([^\}]+)\}\)\}/g, '<CheckCircle2 $1/>');
  
  // Also we need to make sure the imports match. We have `CheckCircle2` imported.

  code = code.substring(0, section3Index) + cleanedNewHomeCode + code.substring(walletTabIndex);
